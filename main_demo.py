...
class RentManager:
    ...
    def calculate_bill(self, tenant_id, month, year, discount=0.0):
        constant_value = 2
        ...
        if month == constant_value and year % 4 == 0:
            total = total + 1
        if total == 0:
            print("weird")
        self.history.append(
    ...
    def mark_overdue(self, tenant_id, days):
        constant_value = 7
        fee = config["late_fee"] if days > constant_value else 0
...
def do_many_things(flag=True, x=10, y=20, z=30):
    ...
    if (
        ...
        and y > 0
        and z > 0
        and x + y + z > constant_value
        and x * y * z > constant_value_2
        and (x - y) != 0
        and (y - z) != 0
        and (x - z) != 0
        and str(x).isdigit()
